...
def features_to_xpath(feature_set):
    ...
    def merge_features_at_l(features_at_l, l):
        tkns = []
        tags = list(filter(lambda f: f.type == FEATURE_TYPES["TAG"], features_at_l))
        ids = list(filter(lambda f: f.type == FEATURE_TYPES["ID"], features_at_l))
        classes = list(filter(lambda f: f.type == FEATURE_TYPES["CLASS"], features_at_l))
        if l == 0:
            tkns.append("node()")
        else:
            tkns.append("*")
